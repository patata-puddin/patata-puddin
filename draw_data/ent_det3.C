...
//   gPad->SetLogy();
  ...
  TTree *TVol2 = (TTree *) P1dfile -> Get("Detector_3");
  Double_t sh_KE;
  Double_t sh_Pdg;
  ...
  // TH3D *hist_sh_2212_KE = new TH3D("Proton_KE", "Proton Energy entering detector3", 100, -225, 225 , 100, -225, 225, 10, 0, 30);
  // TH3D *hist_sh_22_KE = new TH3D("Gamma_KE", "Gamma Energy entering detector3", 100, -225, 225 , 100, -225, 225, 10, 0, 30);
  // TH3D *hist_sh_2112_KE = new TH3D("Neutron_KE", "Neutron Energy entering detector3", 100, -225, 225 , 100, -225, 225, 10, 0, 30);

  TH2D *hist_2212 = new TH2D("Proton", "Proton entering detector3", 100, 1622, 2072 , 100, -225, 225);
  TH2D *hist_22 = new TH2D("Gamma", "Gamma entering detector3", 100, 1622, 2072 , 100, -225, 225);
  TH2D *hist_2112 = new TH2D("Neutron", "Neutron entering detector3", 100, 1622, 2072 , 100, -225, 225);
  ...
  auto Entry_KE = TVol2->GetBranch("pdg")->GetEntries() ;
   ...
   for (j=0;j<Entry_KE; j++){
      TVol2 -> GetEntry(j);
      if( (abs(posx+215) < 0.000000000001)  && ((strcmp(vpvol_p, "Pb_Shield") == 0) ||(strcmp(vpvol_p, "World") == 0))){
            if(sh_Pdg == 2212) {
          // hist_sh_2212_KE -> Fill(posx, posy, sh_KE);  
          hist_2212 -> Fill(posz, posy);  
            }
            if(sh_Pdg == 22) {
          // hist_sh_22_KE -> Fill(posx, posy, sh_KE);  
          hist_22 -> Fill(posz, posy);  

            }
            if(sh_Pdg == 2112) {
          // hist_sh_2112_KE -> Fill(posx, posy, sh_KE);  
          hist_2112 -> Fill(posz, posy);  
            }
      }
   }
