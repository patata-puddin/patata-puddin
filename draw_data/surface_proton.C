...
// TH1D *KE_below = new TH1D("KE_below", "KE of below", 50, 0, 20);
  ...
  TH2D *hist_front_sh = new TH2D("hist_front_sh", "KE of front", 50, 17, 226, 50, -80,130);
  TH2D *hist_back_sh = new TH2D("hist_back_sh", "KE of back", 50, 17, 226, 50, -80,130);
  TH2D *hist_side1_sh = new TH2D("hist_side1_x_sh", "KE of side1", 50, 1817, 1877, 50, -80,130);
  TH2D *hist_side2_sh = new TH2D("hist_side2_y_sh", "KE of side2", 50, 1817, 1877, 50, 17,226);
  TH2D *hist_side3_sh = new TH2D("hist_side3_mx_sh", "KE of side3", 50, 1817, 1877, 50, -80,130);
  TH2D *hist_side4_sh = new TH2D("hist_side4_my_sh", "KE of side4", 50, 1817, 1877, 50, 17,226);

     auto Entry_KE = TVol2->GetBranch("KE")->GetEntries() ;
   for (j=0;j<Entry_KE; j++){
      TVol2 -> GetEntry(j);
if ((sh_Pdg == 22)&& (strcmp(vpvol, "Detector2") != 0)) {
      if(abs(posz-1751)<0.0000001) {
          s_z += 1;
      ...
      } else if(abs(posy-150)<0.0000001) {
          s_y += 1;
      ...
      } else if(abs(posx+225) < 0.0000001) {
          s_mx += 1;
          hist_side3->Fill(posz, posy);
                    if(posz<1840){
            side3_under1840 +=1;
          }
      } else if(abs(posx+235) < 0.0000001) {
          s_x += 1;
...
TCanvas* c1 = new TCanvas("c1", "KE comparision", 20, 20, 1000, 1000);
 ...
 c1->cd(1);
    if (hist_front != nullptr)
    {
        ...
        hist_front->SetTitle("hist of proton at surface -FRONT");
 ...
 c1->cd(2);
    if (hist_back != nullptr)
    {
        ...
        // hist_sh_22_thetaangle_x->SetMinimum(1);
        hist_back->SetTitle("hist of proton at surface -BACK");
        // x_hist_DAQ_2212_KE->SetFillColorAlpha(4,0.1);
        hist_back->Draw("colz");
    }
 c1->cd(3);
